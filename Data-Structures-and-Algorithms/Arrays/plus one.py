def plus_one(digits):
    """
    You are given a large integer represented as an integer array digits, where each digits[i] is the ith digit of
    the integer. The digits are ordered from most significant to least significant in left-to-right order. The large
    integer does not contain any leading 0's. Increment the large integer by one and return the resulting array of
    digits.
    """
    # transform the elements of digits into strings then
    # join them then turn the resulting string into an int
    number = int("".join(map(str, digits)))
    number = number + 1
    # transform number into a string then
    # separate every element and add them to a list
    number = [int(x) for x in str(number)]
    return number
